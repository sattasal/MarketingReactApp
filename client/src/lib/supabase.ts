...
export const supabase = {
  ...
  async deleteWhere(table: string, filter: string) { const r = await fetch(this.url(`${table}?${filter}`), { method: "DELETE", headers: this.headers }); if (!r.ok) throw new Error(await r.text()); },
